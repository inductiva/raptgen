...
from raptgen.data import SequenceGenerator, AugustoSingleRound
...
@click.argument("valid_seqpath", type=click.Path(exists=True))
@click.option("--epochs",
              help="the number of training epochs",
              type=int,
              default=1000)
@click.option("--threshold",
              help="the number of epochs with no loss update to stop training",
              type=int,
              default=50)
...
@click.option("--min-count",
              ...
              default=1)
@click.option("--multi",
              help="the number of training for multiple times",
              type=int,
              default=1)
@click.option(
    "--reg-epochs",
    help="the number of epochs to conduct state transition regularization",
    type=int,
    default=50)
@click.option("--embed-size",
              help="the number of embedding dimension of raptgen model",
              type=int,
              default=2)
@click.option(
    "--fast/--normal",
    help=
    "[experimental] use fast calculation of probability estimation. Output of the decoder shape is different and the visualizers are not implemented.",
    type=bool,
    default=False)
def main(seqpath, valid_seqpath, epochs, threshold, cuda_id, use_cuda,
         save_dir, fwd, rev, min_count, multi, reg_epochs, embed_size, fast):
    ...
    experiment = AugustoSingleRound(path=seqpath,
                                    forward_adapter=fwd,
                                    reverse_adapter=rev)

    experiment_2 = AugustoSingleRound(path=valid_seqpath,
                                      forward_adapter=fwd,
                                      reverse_adapter=rev)

    # training
    train_loader = experiment.get_dataloader(min_count=min_count,
                                             use_cuda=use_cuda)
    test_loader = experiment_2.get_dataloader(min_count=min_count,
                                              use_cuda=use_cuda)
    ...
    for i in range(multi):
        if fast:
            model = CNN_PHMM_VAE_FAST(motif_len=target_len,
